...
| Name                          | Description                                                                                                                                                             |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Name + avatar**             | Extension of multiplayer name entry: display name plus random-default avatar picker. Component target: `EnterNamePage` plus a reusable avatar picker/display component. |
| **Lobby**                     | Flat-player room lobby with invite, readiness, DrawNGuess settings, and avatar-aware player rows. Component target: `RoomLobbyView` + `GameSpecificLobbySections`.      |
| **Custom prompt turn**        | Optional first simultaneous text-entry turn when players create their own starting prompts. Component target: `DrawNGuessCustomPromptTurn`.                             |
| **Draw brief / drawing turn** | Private prompt assignment followed by a controlled whiteboard. Component target: `DrawNGuessDrawingTurn` with `DrawNGuessWhiteboard`.                                   |
