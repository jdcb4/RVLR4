    ...
    players.map((player, playerIndex) => [
      player.id,
      {
        clues: Array.from(
          ...
          (_, clueIndex) => `Clue ${playerIndex}-${clueIndex}`,
        ),
      },
    ]),
  );
...
describe("Hat Game engine", () => {
  it("plays a full game across describe, one-word, and charades", () => {
    ...
    while (session.stage !== "results") {
      if (session.stage === "finalSummary") {
        session = unwrap(applyHatGameAction(session, { type: "view-results" }, actionOptions));
        continue;
      }
  ...
  it("does not complete the next phase from previous phase guesses when a turn ends", () => {
    ...
    expect(session.phaseNumber).toBe(2);
    expect(session.activeTurn?.clueHistory.filter((entry) => entry.phaseNumber === 2)).toHaveLength(
      0,
    );
  ...
  it("scores the active turn when an action detects timeout expiry", () => {
    ...
    session = unwrap(applyHatGameAction(session, { type: "mark-correct" }, actionOptions));
    session = unwrap(
      applyHatGameAction(session, { type: "skip-clue" }, { ...actionOptions, isPast: () => true }),
    );
  ...
  it("reshuffles remaining famous figures before the next turn and avoids the last visible figure first", () => {
    ...
    let session = makeSession({ cluesPerPlayer: 1 });
    session = unwrap(
      applyHatGameAction(session, { type: "start-turn" }, shuffleToPreviousVisibleFirst),
    );

    session = unwrap(
      applyHatGameAction(session, { type: "mark-correct" }, shuffleToPreviousVisibleFirst),
    );
    const lastVisibleFigure = session.activeTurn?.clueQueue[session.activeTurn.queueIndex]?.text;

    session = unwrap(
      applyHatGameAction(session, { type: "end-turn" }, shuffleToPreviousVisibleFirst),
    );
    session = unwrap(
      applyHatGameAction(session, { type: "start-turn" }, shuffleToPreviousVisibleFirst),
    );
  ...
  it("supports selecting one of multiple skipped clues", () => {
    ...
    const currentClue = session.activeTurn?.clueQueue[session.activeTurn.queueIndex];
  ...
  it("validates setup bounds and team sizes", () => {
    ...
    expect(getHatGameSetupError({ playerCount: 4, teamCount: 3 })).toBe(
